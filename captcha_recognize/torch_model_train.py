...
from torch.nn import Linear, ReLU, Softmax, Module, CrossEntropyLoss, Conv2d, MaxPool2d
from torch.nn.init import kaiming_uniform_, xavier_uniform_

CHARACTERS = '0123456789'
IMAGE_WIDTH, IMAGE_HEIGHT, N_LENGTH, N_CLASS = 50, 22, 4, 10
...
class CaptchaDataset(Dataset):
    ...
    def __init__(self):
        ...
        for i, file in enumerate(os.listdir(image_dir)):
            ...
            filename = file[:4]
            self.y1[i] = int(filename[0])
            self.y2[i] = int(filename[1])
            self.y3[i] = int(filename[2])
            self.y4[i] = int(filename[3])
...
def train_model(train_dl, model):
    ...
    for epoch in range(100):
        ...
        for i, (inputs, target_y1, target_y2, target_y3, target_y4) in enumerate(train_dl):
            ...
            # credit assignment
            total_loss.backward()
            print("epoch: {}, batch: {}, total loss: {}, loss1: {}, loss2: {}, loss3: {}, loss4: {}"
                  .format(epoch, i, total_loss.data, loss1.data, loss2.data, loss3.data, loss4.data))
...
def evaluate_model(test_dl, model):
    ...
    for i, (inputs, target_y1, target_y2, target_y3, target_y4) in enumerate(test_dl):
        # evaluate the model on the test set
        pred1, pred2, pred3, pred4 = model(inputs)
